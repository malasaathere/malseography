...
function initApp() {
    ...
    if (toolsContainer) {
        toolsContainer.innerHTML = PORTFOLIO_DATA.tools.map(t => `
            <div class="tool-item">
                <div class="tool-info"><span class="tool-name">${CUSTOM_TOOL_ICONS[t.icon] || `<i data-lucide="${t.icon}" style="width:16px;height:16px;margin-right:8px;vertical-align:middle;color:var(--text-muted);"></i>`}${t.name}</span></div>
